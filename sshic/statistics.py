...
def get_stats(
    ...
    df_stats: pd.DataFrame = pd.DataFrame(columns=[
        "probe", "fragment", "chr", "type", "contacts",
        "coverage_over_hic_contacts", "cis", "trans",
        "intra_chr", "inter_chr"])

    for index, probe in enumerate(probes):
        df_stats.loc[index, "probe"] = probe
        df_stats.loc[index, "fragment"] = df_probes.loc[probe, "frag_id"]
        df_stats.loc[index, "type"] = df_probes.loc[probe, "type"]
    ...
    df_chr_nrm = pd.DataFrame({
        'probes': probes, 'fragments': df_probes["frag_id"].values, 'type': df_probes["type"].values
    })
